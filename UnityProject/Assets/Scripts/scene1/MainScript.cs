...
public class MainScript : MonoBehaviour 
{
	...
	private Rect windowRect = new Rect(20, 20, 200, 0);
	...
	void DoMyWindow(int windowID) 
	{
		...
		Dictionary<string, string> settingsValues = Settings.GetDictionarySettings();
		char[] alphabet = Settings.Instance.alphabet.ToCharArray();

		int count = 0;
		foreach( KeyValuePair<string, string> kvp in settingsValues )
		{
			char c = alphabet[count];
			string s = "("+ c.ToString() + ")  " + kvp.Key + ": " + kvp.Value;
			GUI.Label(new Rect(10, guiTopOffset + count * guiIncrement, 150, 30), s, style);
			count ++;
		}
